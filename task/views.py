...
class TaskUpdateView(generic.UpdateView):
    model = Task
    fields = ["tags"]
    # tags_id = Task.tags.id
    # form_class = TagLicenseUpdateForm
    success_url = reverse_lazy("task:task-list")
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        task = self.get_object()  # Отримати об'єкт завдання
        context['tag_id'] = task.tags.id  # Отримати ідентифікатор тега для завдання
        return context
